window.onload = async () => {
  ...
  console.log("Platform — iOS:", isIOS, "Mobile:", isMobile, "Safari:", isSafari);

  // Fix iOS — speechSynthesis necessite une interaction utilisateur
  ...
  function initVoice() {
    ...
    if (!bootVoice) {
      bootVoice = voices.find(v => v.lang && v.lang.startsWith("en")) || voices[0] || null;
    }
  }
  ...
  function starscreamSpeak(text, onEnd) {
    ...
    // Sur iOS on skip la voix si pas encore debloquee
    if (isIOS && !iosUnlocked) { if (onEnd) onEnd(); return; }
    ...
    // Timeout securite — evite le blocage sur mobile/iOS
    const maxTime = isIOS ? 4000 : isMobile ? 6000 : 9000;
    setTimeout(finish, maxTime);

    // Fix Safari — parfois speechSynthesis se bloque en arriere plan
    if (isSafari || isIOS) {
      const resume = setInterval(() => {
        if (window.speechSynthesis.paused) window.speechSynthesis.resume();
      }, 250);
      utter.onend = () => { clearInterval(resume); finish(); };
      utter.onerror = () => { clearInterval(resume); finish(); };
    }
  ...
  const BOOT_CHECKS = [
    {
      ...
      errPhrase: "Warning. Firebase database module failed to load."
    },
    {
    ...
    },
    {
      ...
      errPhrase: "Critical failure. GPS module is offline."
    },
    {
      label: "LOCAL STORAGE",
      key: "storage",
      ...
      errPhrase: "Storage module degraded."
    },
    {
      label: "AUDIO ENGINE",
      key: "audio",
      test: async () => (window.AudioContext || window.webkitAudioContext) ? "ok" : "warn",
      errPhrase: "Audio engine unavailable."
    },
    {
      label: "STAR BRIDGE",
      key: "star",
  ...
  // Timeout securite global — force le lancement selon la plateforme
  const bootTimeout = isIOS ? 12000 : isMobile ? 16000 : 25000;
  setTimeout(() => {
    const overlay = document.getElementById("bootOverlay");
    if (overlay && overlay.style.display !== "none") {
      console.warn("BOOT TIMEOUT — forcing HUD launch");
      launchHUD();
    }
  }, bootTimeout);
  ...
  async function runBootSequence() {
    ...
    // Sur iOS on attend le premier toucher pour lancer la voix
    if (isIOS) {
      if (voiceTextEl) voiceTextEl.innerText = "TAP TO INITIALIZE...";
    ...
    if (errors.length === 0 && warnings.length === 0) {
      await speak("All systems nominal. STAR bridge active. Decepticons... prepare for battle.");
    } else if (errors.length === 0) {
      await speak("Primary systems online. STAR bridge active. Stay sharp.");
    } else {
      await speak("System boot completed with failures. Proceeding under compromised conditions.");
    }
  ...
  function initMap() {
    ...
    try {
      map = L.map("miniMap").setView([49.6116, 6.1319], 13);
      L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: "© OpenStreetMap"
      }).addTo(map);
      // Ajouter le marqueur cible initial
      if (TARGET) {
        window._targetMarker = L.circleMarker([TARGET.lat, TARGET.lon], {
          radius: 8, color: "#00ff88", fillColor: "#00ff88", fillOpacity: 0.8
  ...
  db.ref("techstar/commands/current").on("value", async snapshot => {
    ...
    console.log("STAR CMD:", cmd.action, cmd.param);
    ...
    try {
      switch (cmd.action) {
        case "seeker_mode": {
          const btn = document.getElementById("enterSeekerBtn");
          if (btn) btn.click();
          message = "seeker mode activated";
          break;
        }
        case "exit_seeker": {
          const btn = document.getElementById("exitSeekerBtn");
          if (btn) btn.click();
          message = "seeker mode exited";
          break;
        }
        case "chill_mode": {
          const btn = document.getElementById("enterChillBtn");
          if (btn) btn.click();
          message = "chill mode activated";
          break;
        }
        case "exit_chill": {
          const btn = document.getElementById("exitChillBtn");
          if (btn) btn.click();
          message = "chill mode exited";
          break;
        }
        case "free_mode": {
          const btn = document.getElementById("enterFreeBtn");
          if (btn) btn.click();
        ...
        case "set_target": {
          if (cmd.param) {
            ...
            if (data && data.length > 0) {
              ...
              if (map) {
                if (window._targetMarker) map.removeLayer(window._targetMarker);
                window._targetMarker = L.circleMarker([TARGET.lat, TARGET.lon], {
                  radius: 8, color: "#00ff88", fillColor: "#00ff88", fillOpacity: 0.8
                }).addTo(map).bindPopup("TARGET: " + TARGET.name);
              }
        ...
        case "get_status": {
          message = JSON.stringify({
            lat: myLat ? myLat.toFixed(5) : "0",
            lon: myLon ? myLon.toFixed(5) : "0",
    ...
    db.ref("techstar/status/last").set({
      action: cmd.action, ok: ok, message: message, updated: Date.now()
    });
  });
  ...
  function pushStatusToSTAR() {
    ...
    db.ref("techstar/radar_data").set({
      ...
      target: TARGET.name, updated: Date.now()
    });
  }
  ...
  const TARGET = savedTarget ? JSON.parse(savedTarget) : {
    name: "LUXEMBOURG CITY", lat: 49.6116, lon: 6.1319
  };

  const savedCallsign = localStorage.getItem("callsign");
  const savedSquad    = localStorage.getItem("squad");

  // Sur mobile/iOS on evite prompt() bloquant si deja sauvegarde
  let playerCode, squadCode;
  if (isMobile && savedCallsign) {
    playerCode = savedCallsign;
    squadCode  = savedSquad || "PUBLIC";
  } else {
    playerCode = (prompt("ENTER CALLSIGN", savedCallsign || "") || "UNKNOWN").trim().toUpperCase();
    squadCode  = (prompt("ENTER SQUAD",    savedSquad    || "") || "PUBLIC").trim().toUpperCase();
  }
  ...
  function savePlayer(lat, lon, altitude = 0, speed = 0) {
    ...
    if (map) {
      if (!myMarker) {
        myMarker = L.circleMarker([lat, lon], {
          radius: 8, color: "#bb66ff", fillColor: "#bb66ff", fillOpacity: 0.9
    ...
    db.ref("players/" + currentUid).set({
      name: playerCode, squad: squadCode,
      lat: lat, lon: lon, updated: Date.now()
    ...
    navigator.geolocation.watchPosition(
      pos => {
        ...
        savePlayer(
          ...
          pos.coords.altitude || 0, pos.coords.speed || 0
        );
      },
  ...
  db.ref("players").on("value", snapshot => {
    ...
    for (let id in data) {
      ...
      onlinePlayers.push({
        name: p.name || "UNKNOWN", squad: p.squad || "NO SQUAD",
        lat: p.lat || 0, lon: p.lon || 0,
        sameSquad: sameSquad, uid: id, self: id === currentUid
      });
    }
  ...
  if (enterSeekerBtn) {
    enterSeekerBtn.onclick = () => {
      ...
      fill.style.width        = "0%";
      const messages = [
        "SEEKER MODE INITIALIZING...", "LOADING RADAR SYSTEMS...",
        "ACQUIRING GPS SIGNAL...", "TARGET LOCKED...", "ENGAGING SEEKER MODE..."
      ];
      let progress = 0;
      let msgIndex = 0;
  ...
  window.addEventListener("resize", () => { if (seekerActive) resizeSeeker(); });
  ...
  function resizeChillRadar() {
    ...
    chillRadarCanvas.width = 180; chillRadarCanvas.height = 180;
    chillRadarCx = 90; chillRadarCy = 90;
  }
  ...
  if (enterChillBtn) {
    enterChillBtn.onclick = async () => {
      chillActive = true; chillStartTime = Date.now();
      chillDistanceTotal = 0; chillLastLat = myLat; chillLastLon = myLon;
      chillMode.style.display = "block";
      resizeChillRadar(); resizeVisualizer();
      await startAudio();
      if (myLat !== 0) {
  ...
  function resizeFreeRadar() {
    ...
    freeRadar.width = size; freeRadar.height = size;
    freeRadarCx = size / 2; freeRadarCy = size / 2;
  }

  function drawFreeRadar() {
    if (!freeActive || !freeRadarCtx) return;
    const W = freeRadar.width; const H = freeRadar.height;
    freeRadarCx = W / 2; freeRadarCy = H / 2;
    const t = FREE_THEMES[freeTheme]; const rgb = t.main;
    freeRadarCtx.clearRect(0, 0, W, H);
    for (let i = 1; i <= 5; i++) {
      freeRadarCtx.beginPath();
      freeRadarCtx.arc(freeRadarCx, freeRadarCy, i * (W / 11), 0, Math.PI * 2);
      freeRadarCtx.strokeStyle = `rgba(${rgb},0.3)`; freeRadarCtx.lineWidth = 1; freeRadarCtx.stroke();
    }
    for (let i = 0; i < 40; i++) {
      ...
      freeRadarCtx.strokeStyle = `rgba(${rgb},${1 - i / 40})`; freeRadarCtx.lineWidth = 2; freeRadarCtx.stroke();
    }
    onlinePlayers.forEach(player => {
      const range = 0.02;
      const dx = ((player.lon - myLon) / range) * (W / 2);
      const dy = ((player.lat - myLat) / range) * -(H / 2);
      const px = freeRadarCx + dx; const py = freeRadarCy + dy;
      ...
      freeRadarCtx.fillStyle = color; freeRadarCtx.shadowBlur = 10; freeRadarCtx.shadowColor = color;
      freeRadarCtx.fill(); freeRadarCtx.shadowBlur = 0;
    });
    freeRadarCtx.beginPath();
    freeRadarCtx.arc(freeRadarCx, freeRadarCy, 6, 0, Math.PI * 2);
    freeRadarCtx.fillStyle = t.hex; freeRadarCtx.shadowBlur = 16; freeRadarCtx.shadowColor = t.hex;
    freeRadarCtx.fill(); freeRadarCtx.shadowBlur = 0;
    freeAngle += 0.018;
  ...
  function updateFreeSpotify() {
    ...
    el.innerHTML = `<div class="free-spotify-inner"><div class="free-spotify-track">${track}</div><div class="free-spotify-artist">${artist}</div></div>`;
  }

  let freeLoopInterval = null;

  function startFreeLoop() {
    updateFreeGps(); updateFreeSpotify(); freeLog("FREE MODE ACTIVE");
    freeLoopInterval = setInterval(() => { updateFreeGps(); updateFreeSpotify(); }, 3000);
  }
  ...
  if (enterFreeBtn) {
    enterFreeBtn.onclick = () => {
      ...
      freeMode.style.display = "block"; freeActive = true; _lastFreeGpsStr = "";
      applyFreeTheme(freeTheme); resizeFreeRadar(); drawFreeRadar(); startFreeLoop();
    };
  }

  if (exitFreeBtn) {
    exitFreeBtn.onclick = () => {
      freeActive = false; stopFreeLoop();
      freeMode.style.display = "none";
      document.querySelector(".hud").style.display = "flex";
    };
  }

  window.addEventListener("resize", () => { if (freeActive) resizeFreeRadar(); });
  ...
  async function spotifyFetch(endpoint, method = "GET", body = null) {
    ...
    if (res.status === 401) {
      spotifyToken = null; localStorage.removeItem("spotify_token");
